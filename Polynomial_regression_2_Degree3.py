...
x_new_Linear = np.array([10]).reshape(-1,1)
y_pred_Linear = modeLinear.predict(x_new_Linear)

x1_Linear = np.linspace(10,20,100).reshape(-1,1)
y1_Linear = modeLinear.predict(x1_Linear)

plt.plot(x1_Linear,y1_Linear,color='aqua', label = ("Y = %.2fX + %.2f,(R2 = %.2f)"%(bL ,aL,r2_linear)))
